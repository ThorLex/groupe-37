  ...
  try {
    ...
    client.messages
      .create({
        ...
        to: '+221771234567' // A remplacer par le numéro du client
      })
...
router.post('/momo-pay', async (req, res) => {
  try {
    ...
    const response = await momoCollection.requestToPay({
      ...
      payerMessage,
      payeeNote,
    }, accessToken); // Passer le token d'accès
    res.json({ message: 'Paiement Momo initié', data: response });
  } catch (err) {
    res.status(400).json({ message: 'Erreur paiement Momo', error: err.message });
  }
});
...
router.post('/momo-callback', async (req, res) => {
  try {
    ...
    console.log(`Momo Callback received: Status - ${status}, External ID - ${externalId}`);
    ...
    res.status(500).json({ message: 'Erreur traitement callback Momo', error: err.message });
  }
});
